...
gradient_step = 1
hidden_dim = 256
entropy_control = False     # if True, reward will be fluctuated
buffer_size = 100000         # 1M = 1000000

project_name = f'pre-trained_uav/SAC_{REWARD_DESIGN}' if TRAINED_UAV else \
    f'SAC/{args.algo}_{REWARD_DESIGN}_batch{batch_size}_step{gradient_step}_ep{episode_num}_hd{hidden_dim}_ent{entropy_rate}_entDecay{entropy_control}_lr{learning_rate}_buffer{buffer_size}'
...
while episode_cnt < episode_num:
    ...
    while step_cnt < step_num:
        ...
        if not system.render_obj.pause:
            ...
            # 5 update agent after buffer is enough
            if episode_cnt >= 3:
