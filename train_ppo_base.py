...
agent_ris = PPO(
    state_dim = system.get_system_state_dim(),
    action_dim = system.get_system_action_dim() - 2,
    lr_actor = 5e-5,
    lr_critic = 1e-4,
    gamma = 0.99,
    K_epochs = 5,
...
while episode_cnt < episode_num:
    ...
    while step_cnt < step_num:
        ...
        if not system.render_obj.pause:
            ...
            action_ris = agent_ris.select_action(observarion_ris)  # 0.1 is action noise factor
            action_uav = agent_uav.select_action(observarion_uav)
            if if_BS:
                action_uav[0] = 0
                action_uav[1] = 0
            ...
            agent_ris.buffer.rewards.append(reward)
            agent_uav.buffer.rewards.append(reward)

            if done == True:
                agent_ris.buffer.is_terminals.append(done)
                agent_uav.buffer.is_terminals.append(done)
            else:
                agent_ris.buffer.is_terminals.append(done)
                agent_uav.buffer.is_terminals.append(done)

            # 5 update PPO by old policy
            if episode_cnt != 0 and episode_cnt % 10 == 0 and step_cnt == 100:
                agent_ris.update()
                if not TRAINED_UAV:
                    agent_uav.update()

            if episode_cnt != 0 and episode_cnt % 10 == 0 and step_cnt == 100:
                agent_ris.decay_action_std(1e-3, 0.01)
