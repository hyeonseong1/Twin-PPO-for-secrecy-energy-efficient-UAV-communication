...
class PPOAgent(object):
    def __init__(self,
        ...
        self.memory = PrioritizedReplayBuffer(max_size, input_dim, n_action)
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            mu_old, std_old = self.old_actor(old_states)
            old_log_probs = -0.5 * (
        ...
        actor_loss.backward()
        # self.clip_grad_with_min_norm(self.actor, 100, 1e-6)
        ...
        # element-wise MSE loss를 구한 후, weights를 곱하고 평균을 취함.
        critic_loss = (F.mse_loss(self.critic(old_states), critic_target, reduction='none') * torch.tensor(weights,
                                                                                                           device=self.actor.device)).mean()
        ...
        new_priorities = (deltas.abs().detach().cpu().numpy() + self.epsilon).flatten()
        self.memory.update_priorities(tree_idxs, new_priorities)

    # I expected the problem to be caused by gradient vanishing, but it was actually gradient exploding
    def clip_grad_with_min_norm(self, model, max_norm, min_norm):
        """
        기울기의 최댓값은 `max_norm`으로, 최솟값은 `min_norm`으로 보장하는 함수.
        """
        # 모든 파라미터의 기울기 벡터들을 하나로 합쳐서 L2 노름 계산
        total_norm = torch.norm(torch.cat([p.grad.view(-1) for p in model.parameters()]))

        # 기울기가 최댓값을 초과하면 클리핑
        if total_norm > max_norm:
            nn.utils.clip_grad_norm_(model.parameters(), max_norm)

        # 기울기가 최솟값을 아래로 내려가지 않도록 조정
        total_norm = torch.norm(torch.cat([p.grad.view(-1) for p in model.parameters()]))
        if total_norm < min_norm:
            # 기울기를 min_norm 크기로 설정
            scale = min_norm / total_norm
    ...
    def reset(self):
        self.memory = PrioritizedReplayBuffer(self.max_size, self.input_dim, self.n_action)
